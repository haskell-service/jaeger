...
-- Stability:   alpha
-- Portability: FlexibleContexts, FlexibleInstances, GeneralizedNewtypeDeriving, MultiParamTypeClasses, StandaloneDeriving, TypeFamilies, UndecidableInstances
--
...
instance MonadBase IO m => MonadJaeger (JaegerT m) where
    emitSpan s = do
        env <- JaegerT ask
        -- Make sure it's evaluated before entering a 'critical section'. Not
        -- sure this is strictly required.
        estimatedSpanSize `seq` liftBase $ join $ atomically $ do
...
runJaegerT :: (MonadBase IO m, MonadMask m) => JaegerT m a -> Socket -> Process -> Sampler IO -> m a
runJaegerT act s p r = do
    (tv, mtu) <- liftBase $ (,) <$> newTVarIO (0, []) <*> getMTU s
    let env = Env s mtu p r tv
    runReaderT (unJaegerT act) env `finally` liftBase (flush tv)
  where
    flush tv = do
